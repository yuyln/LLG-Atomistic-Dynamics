...
def unstructured():
    with open("test.vtk", "w") as f:
        ...
        f.write(f"FIELD angle_field 1\n")
        f.write(f"angle 1 {gi.rows * gi.cols * gi.depth} float\n")
        for z in range(gi.depth):
            for y in range(gi.rows):
                for x in range(gi.cols):
                    f.write(f"{np.arctan2(my[z, y, x], mx[z, y, x])}\n")
def rectilinear():
    with open("test.vtk", "w") as f:
        f.write("# vtk DataFile Version 2.0\n")
        f.write("Lattice\n")
        ...
        f.write(f"X_COORDINATES {gi.cols} float\n")
        for x in range(gi.cols):
            f.write(f"{x:.1f} ")
        f.write("\n")
    
        f.write(f"Y_COORDINATES {gi.rows} float\n")
        for y in range(gi.rows):
            f.write(f"{y:.1f} ")
        f.write("\n")
    
        f.write(f"Z_COORDINATES {gi.depth} float\n")
        for z in range(gi.depth):
            f.write(f"{z:.1f} ")
        f.write("\n")
