...
char *create_current_she_dc(double j, v3d p, double beta) {
    ...
    p = v3d_normalize(p);
    p = v3d_scalar(p, j);
    int written = snprintf(buffer, sizeof(buffer) - 1, "return (current){.type = CUR_SHE, .she.p = v3d_c(%.15e, %.15e, %.15e), .she.theta_sh = -1, .she.thickness = 2.0 * gs.lattice, .she.beta= %.15e};", p.x, p.y, p.z, beta);
...
char *create_current_she_ac(double j, v3d p, double omega, double beta) {
    ...
    p = v3d_normalize(p);
    p = v3d_scalar(p, j);
    int written = snprintf(buffer, sizeof(buffer) - 1, "return (current){.type = CUR_SHE, .she.p = v3d_c(%.15e * cos(2.0 * M_PI * %.15e * time), %.15e * sin(2.0 * M_PI * %.15e * time), %.15e * sin(2.0 * M_PI * %.15e * time)), .she.theta_sh = -1, .she.thickness = 2.0 * gs.lattice, .she.beta= %.15e};", p.x, omega, p.y, omega, p.z, omega, beta);
...
char *create_current_she_dc_ac(double j_dc, v3d p_dc, double j_ac, v3d p_ac, double omega, double beta) {
    ...
    int written = snprintf(buffer, sizeof(buffer) - 1, "return (current){.type = CUR_SHE, .she.p = v3d_c(%.15e + %.15e * cos(2.0 * M_PI * %.15e * time), %.15e + %.15e * sin(2.0 * M_PI * %.15e * time), %.15e + %.15e * sin(2.0 * M_PI * %.15e * time)), .she.theta_sh = -1, .she.thickness = 2.0 * gs.lattice, .she.beta= %.15e};", p_dc.x, p_ac.x, omega, p_dc.y, p_ac.y, omega, p_dc.z, p_ac.z, omega, beta);
