...
int Stosic(void) {
    ...
    int_params.dt = dt * 0.1;
    grid_renderer_integrate(&g, int_params, 1000 / ratio, 1000);
...
void set_pin(grid *g, uint64_t row, uint64_t col, void *dummy) {
    ...
    //                                             .dmv_left = v3d_c(0.0, -dm, 0.0),
    //                                             .dmv_right = v3d_c(0.0, dm, 0.0)};
    //grid_set_dm_loc(g, row, col, default_dm);
    int center = 32;
    int dx = col - center;
    int dy = row - center;
    double d2 = dx * dx + dy * dy;
    double size = it->size / 3.0;
   // grid_set_anisotropy_loc(g, row, col, (anisotropy){.ani = -20 * it->J * exp(-d2 / (size * size)), .dir = v3d_c(0, 0, 1)});
    grid_set_exchange_loc(g, row, col, it->J * 5);
...
int test(void) {
    unsigned int rows = 272;
    unsigned int cols = 272;
    ...
    string current_func = str_is_cstr("current ret = (current){};\n"\
                                      "time -= 0.2 * NS;\n"\
                                      "ret.type = CUR_STT;\n"\
                                      "ret.stt.j = v3d_c(1e10 * cos(time / (NS * 10)), 1e10 * sin(time / (NS * 10)), 0.0);\n"\
                                      "ret.stt.beta = 0.0;\n"\
                                      "ret.stt.polarization = -1.0;\n"\
                                      "ret.stt.j = v3d_scalar(ret.stt.j, (time > 0));\n"\
                                      "return ret;");
